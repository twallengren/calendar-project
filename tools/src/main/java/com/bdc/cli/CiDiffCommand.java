...
public class CiDiffCommand implements Callable<Integer> {
    ...
    @Option(names = {"--blessed-dir", "-b"}, description = "Path to blessed artifacts directory", defaultValue = "blessed")
    private Path blessedDir;

    @Option(names = {"--calendars", "-c"}, description = "Comma-separated calendar IDs or 'all'", defaultValue = "all")
    private String calendars;

    @Option(names = {"--from"}, description = "Override range start date")
    private LocalDate fromOverride;

    @Option(names = {"--to"}, description = "Override range end date")
    private LocalDate toOverride;

    @Option(names = {"--output-format", "-f"}, description = "Output format: json or markdown", defaultValue = "json")
    private String outputFormat;

    @Option(names = {"--cutoff-date"}, description = "Cutoff date for historical vs future (default: today)")
    ...
    @Option(names = {"--current-sha"}, description = "Current git SHA for report")
    ...
    public Integer call() {
        try {
            ...
            for (String calendarId : calendarIds) {
                ...
                if (calInfo == null) {
                    System.err.println("Warning: Calendar " + calendarId + " not found in manifest, skipping");
                    continue;
                }
                ...
                // Compare
                CalendarDiff diff = diffEngine.compare(calendarId, generatedEvents, blessedEvents, cutoffDate);
