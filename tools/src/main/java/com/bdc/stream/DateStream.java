 ...
 *
 * DateStream provides methods for querying calendar events and business days.
 * Implementations may be lazy (compute on demand) or materialized (pre-computed).
 */
public interface DateStream {
    ...
    /**
     * Get the event on a specific date, if any.
     * If multiple events exist on the same date, returns the first one.
     */
    ...
    /**
     * Find the nth business day relative to the given date.
