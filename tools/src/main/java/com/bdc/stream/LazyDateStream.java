 ...
 *
 * Events are generated on-the-fly when requested, using the provided ResolvedSpec.
 * This is efficient for queries but may be slower for repeated access patterns.
 * A small cache is maintained to improve performance for repeated queries.
 */
public class LazyDateStream implements DateStream {
    ...
    private final Map<LocalDate, List<Event>> dayCache = new LinkedHashMap<>(100, 0.75f, true) {
        @Override
    ...
    public LazyDateStream(ResolvedSpec spec) {
        ...
        this.weekendDays = spec.weekendPolicy() != null
    ...
    public List<Event> eventsOn(LocalDate date) {
        return dayCache.computeIfAbsent(date, d ->
            generator.generate(spec, d, d));
    }
    ...
    public LocalDate prevBusinessDay(LocalDate from) {
        ...
        if (maxDays == 0 && !isBusinessDay(candidate)) {
            throw new IllegalStateException("Could not find a business day within 30 days before " + from);
        }
        return candidate;
    }
    ...
    public long businessDaysInRange(LocalDate from, LocalDate to) {
        ...
        List<Event> closedEvents = eventsInRange(from, to).stream()
