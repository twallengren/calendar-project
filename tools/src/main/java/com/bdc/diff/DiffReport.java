...
public record DiffReport(
  ...
  public static DiffReport create(
      Map<String, CalendarDiff> calendars, String blessedVersion, String currentSha) {
    DiffSeverity overall =
        calendars.values().stream()
            .map(CalendarDiff::severity)
            .reduce(DiffSeverity.NONE, DiffReport::maxSeverity);

    int withChanges = (int) calendars.values().stream().filter(CalendarDiff::hasChanges).count();
