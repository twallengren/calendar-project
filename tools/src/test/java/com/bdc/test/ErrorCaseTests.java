...
/**
 * Error case tests for calendar generation. Tests that the system properly handles and reports
 * errors.
 */
class ErrorCaseTests {
  ...
  void missingCalendarReference() {
    ...
    assertTrue(
        ex.getMessage().contains("Calendar not found") || ex.getMessage().contains("NONEXISTENT"),
  ...
  void missingModuleReference() {
    ...
    assertTrue(
        ex.getMessage().contains("Module not found") || ex.getMessage().contains("nonexistent"),
  ...
  void invertedDateRangeThrows() {
    ...
    assertTrue(
        ex.getMessage().contains("start must not be after end"),
  ...
  void invalidMonthInFixedMonthDay() {
    ...
    // Month 13 is invalid
    Rule.FixedMonthDay invalidRule =
        new Rule.FixedMonthDay("invalid", "Invalid Holiday", 13, 1, "ISO");
    ...
    assertTrue(occurrences.isEmpty(), "Invalid month should result in no occurrences");
  }
  ...
  void invalidDayInFixedMonthDay() {
    ...
    Rule.FixedMonthDay invalidRule =
        new Rule.FixedMonthDay("invalid", "Invalid Holiday", 2, 31, "ISO");

    var occurrences = expander.expand(invalidRule, range, "test");

    // Invalid day should result in no valid occurrences
    assertTrue(occurrences.isEmpty(), "Invalid day should result in no occurrences");
  }
  ...
  void nthWeekdayDoesntExist() {
    ...
    DateRange range = new DateRange(LocalDate.of(2024, 2, 1), LocalDate.of(2024, 2, 29));

    // 5th Monday of February 2024 doesn't exist
    Rule.NthWeekdayOfMonth rule =
        new Rule.NthWeekdayOfMonth("fifth_monday", "Fifth Monday", 2, DayOfWeek.MONDAY, 5);

    var occurrences = expander.expand(rule, range, "test");

    assertTrue(occurrences.isEmpty(), "5th Monday of February 2024 should not exist");
  }
  ...
  void nthWeekdayZeroReturnsEmpty() {
    ...
    Rule.NthWeekdayOfMonth rule =
        new Rule.NthWeekdayOfMonth("zeroth_monday", "Zeroth Monday", 1, DayOfWeek.MONDAY, 0);

    var occurrences = expander.expand(rule, range, "test");

    assertTrue(occurrences.isEmpty(), "nth=0 should result in no occurrences");
  }
  ...
  void nullCalendarIdThrows() {
    assertThrows(
        Exception.class, () -> resolver.resolve(null), "Should throw for null calendar ID");
  }
