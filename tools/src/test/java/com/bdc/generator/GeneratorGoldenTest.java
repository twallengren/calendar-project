...
class GeneratorGoldenTest {
    ...
    void generateUSMarketBase2024() {
        ...
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 1, 1)) && e.description().equals("New Year's Day")
        ));
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 5, 27)) && e.description().equals("Memorial Day")
        ));
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 7, 4)) && e.description().equals("Independence Day")
        ));
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 9, 2)) && e.description().equals("Labor Day")
        ));
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 11, 28)) && e.description().equals("Thanksgiving Day")
        ));
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 12, 25)) && e.description().equals("Christmas Day")
        ));
    }
    ...
    void generateNYSECalendar() {
        ...
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 1, 15)) && e.description().equals("Martin Luther King Jr. Day")
        ));

        // Should include Good Friday
        assertTrue(events.stream().anyMatch(e ->
            e.date().equals(LocalDate.of(2024, 3, 29)) && e.description().equals("Good Friday")
        ));
    }
}
