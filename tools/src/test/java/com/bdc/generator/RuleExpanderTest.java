...
class RuleExpanderTest {
    ...
    void expandFixedMonthDay() {
        Rule.FixedMonthDay rule = new Rule.FixedMonthDay(
            "christmas", "Christmas Day", 12, 25, "ISO"
        );
    ...
    void expandNthWeekdayOfMonth_first() {
        ...
        Rule.NthWeekdayOfMonth rule = new Rule.NthWeekdayOfMonth(
            "labor_day", "Labor Day", 9, DayOfWeek.MONDAY, 1
        );
    ...
    void expandNthWeekdayOfMonth_last() {
        ...
        Rule.NthWeekdayOfMonth rule = new Rule.NthWeekdayOfMonth(
            "memorial_day", "Memorial Day", 5, DayOfWeek.MONDAY, -1
        );

        List<Occurrence> occurrences = expander.expand(rule, range2024, "test");

        assertEquals(1, occurrences.size());
        assertEquals(LocalDate.of(2024, 5, 27), occurrences.get(0).date()); // May 27, 2024 is last Monday
    }
    ...
    void expandNthWeekdayOfMonth_fourth() {
        ...
        Rule.NthWeekdayOfMonth rule = new Rule.NthWeekdayOfMonth(
            "thanksgiving", "Thanksgiving Day", 11, DayOfWeek.THURSDAY, 4
        );
    ...
    void expandExplicitDates() {
        Rule.ExplicitDates rule = new Rule.ExplicitDates(
    ...
    void expandHijriFixedMonthDay() {
        ...
        Rule.FixedMonthDay rule = new Rule.FixedMonthDay(
            "eid_al_fitr", "Eid al-Fitr", 10, 1, "HIJRI"
        );

        List<Occurrence> occurrences = expander.expand(rule, range2024, "test");

        // Should find Eid al-Fitr 1445 (approx April 2024) and 1446 (approx March/April 2025 - outside range)
        assertFalse(occurrences.isEmpty());
        // Eid al-Fitr 1445 falls on approximately April 10, 2024
        assertTrue(occurrences.stream().anyMatch(o ->
            o.date().getYear() == 2024 && o.date().getMonthValue() == 4
        ));
    }
}
