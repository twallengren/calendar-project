...
class PropertyBasedTest {
  ...
  private static final Path TEST_MODULES_DIR =
  ...
  void eventsAreSortedByDate(
      @ForAll("validCalendarIds") String calendarId, @ForAll("years") int year) throws Exception {
    ...
    List<Event> sorted = events.stream().sorted().toList();
  ...
  void noEventsOutsideRange(
      ...
      @ForAll @IntRange(min = 1, max = 12) int endMonth)
      throws Exception {
  ...
  @Property(tries = 50)
  void deterministicOutput(@ForAll("validCalendarIds") String calendarId, @ForAll("years") int year)
      throws Exception {
  ...
  void diamondDependencyProducesUniqueEvents() throws Exception {
    ...
    long sharedCount =
        events.stream().filter(e -> e.description().equals("Shared Holiday")).count();
  ...
  void deepInheritancePreservesAllEvents() throws Exception {
    ...
    assertTrue(
        events.stream().anyMatch(e -> e.description().equals("Level A Event")),
        "Should have Level A Event from base");
    assertTrue(
        events.stream().anyMatch(e -> e.description().equals("Level B Event")),
        "Should have Level B Event from middle");
    assertTrue(
        events.stream().anyMatch(e -> e.description().equals("Deep Inheritance Event")),
  ...
  void yearBoundaryEventsInSingleYearRange() throws Exception {
    ...
    assertTrue(
        events2024.stream().anyMatch(e -> e.date().equals(LocalDate.of(2024, 1, 1))),
        "Should have New Year's Day 2024");
    assertTrue(
        events2024.stream().anyMatch(e -> e.date().equals(LocalDate.of(2024, 12, 31))),
  ...
  void yearBoundaryEventsAcrossYears() throws Exception {
    ...
    List<Event> events =
        generator.generate(spec, LocalDate.of(2024, 12, 1), LocalDate.of(2025, 1, 31));
